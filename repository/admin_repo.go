...
func (a *admin_repo) GetAllAvailableRoom() ([]model.BoardingRoom, error) {
	var data []model.BoardingRoom
	err := a.sqlxDb.Select(&data, "select * from boarding_room where is_available = true")
	if err != nil {
		return nil, err
	}
	fmt.Println(data)
...
func (a *admin_repo) GetAllCustomer() ([]model.Customer, error) {
	var data []model.Customer
	err := a.sqlxDb.Select(&data, "select name, address, ktpid, phone_number, start_rent_at, end_rent_at from customer")
	if err != nil {
		return nil, err
	}
	return data, nil
